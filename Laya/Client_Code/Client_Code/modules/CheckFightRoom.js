...
// import * as CZJump from "../dati_comm/modules/CZJump";
...
//战斗房间
class CheckFightRoom
{
  constructor()
  {
    console.log('=========================>init CheckFightRoom')
    GameConn.OnNotify.On(this, this.onGameRecv)
    this.EvtCheckDoLun0 = new UIEvent()
    this.EvtCheckDoLun = new UIEvent()
    this.EvtCheckktend = new UIEvent()
    this.EvtCheckT = new UIEvent()
    this.EvtCheckLock = new UIEvent()
  ...
  //当前是否为3v3模式
  get Is3V3() { 
    return this.CurrMode == 4 || this.CurrMode == 9   
  } 

  //当前是否为邀请好友模式
  get IsYaoQing() {
    return this.CurrMode == 9 || this.CurrMode == 8 
  }
  ...
  onGameRecv(jDoc){
    console.log('================> game recv')
    switch(jDoc.n){
      case 'lun0':
      {
        this._ParseChecklun0(jDoc)
      }
      break;

      case 'lun':
      {
        this._ParseChecklun(jDoc)
      }
      break;

      case 'ktend':
      {
        this._ParseCheckktend(jDoc)
      }
      break;

      case 't':
      {
        this._ParseCheckT(jDoc)
      }
      break;

      case 'cglock':
      {
        this._ParseCheckLock(jDoc)
      }
      break
    }
  }

  _ParseChecklun0(jDoc){
    console.log('parse lun0', jDoc)
    this.Lun = jDoc.currlun
    this.DatiTime = jDoc.dt //答题时间，秒
    this.LastTime = jDoc.t
    this.EvtCheckDoLun0.Emit()
  }

  _ParseChecklun(jDoc){
    console.log('parse lun', jDoc)
    ...
    this.EvtCheckDoLun.Emit()
  }

  _ParseCheckktend(jDoc){
    console.log('parse kantiend', jDoc)
    this.Daan = jDoc.daan //答案数组
    this.errBtnArr = jDoc.err
    this.EvtCheckktend.Emit()
  }

  _ParseCheckT(jDoc){
    console.log('parse time', jDoc)
    this.SYDatiTime = jDoc.v
    this.EvtCheckT.Emit()
  }

  _ParseCheckLock(jDoc){
    console.log('parse parse', jDoc)
    this.EvtCheckLock.Emit(jDoc)
  }
  ...
  LoginClear()
  {
    ...
    {
      CheckFightRoom.Fighting = true//设置正在战斗的标记，其它页面会负责发送退出战斗指令
    }else
    { 
      CheckFightRoom.Fighting = false//清当前正在战斗的标记
    }
  }
...
instance = new CheckFightRoom()
export { instance as CheckFightRoom, StopFight }
