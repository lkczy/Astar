...
(function (mpsdk) {
    ...
    var Env = /** @class */ (function () {
        ...
        Env.onlineTimeTotal = 0;
        Env.onlineTimeCount = 0;
        return Env;
    }());
    mpsdk.Env = Env;
})(mpsdk || (mpsdk = {}));
...
(function (mpsdk) {
     ...
     * gender: -1不限制性别/1男/2女/0未知
        ...
        Ad.checkCityVisible = function (hackData, city) {
            ...
            mpsdk.log('广告可见城市：', city, ' ; 当前城市：', currentCity);
...
(function (mpsdk) {
     ...
     * wx.onHide(mpsdk.Ad.outCheck)
        ...
        Hack.isStrongFission = function (fromAppId) {
            ...
            var senceArray = [1000, 1005, 1006, 1011, 1012, 1013, 1017, 1025, 1027, 1030, 1031, 1032, 1042, 1047, 1048, 1049, 1053, 1054, 1084, 1106, 2001, 2970];
            ...
            mpsdk.log('服务器端强烈变检查：强烈变状态：', isServerStrongFission, ' ; 当前场景值：', serverSence, ' ; 当前本地场景值：', mpsdk.Env.launchOptions.scene);
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            _a.trys.push([0, 2, , 3]);
                            // 获取当前时长
                            // const onlineTimeTotal = Number(Platform.instance.getStorage('online_time_total') || 0) + 1;//累计时长
                            // const onlineTimeCount = Number(Platform.instance.getStorage('online_time_count') || 0) + 1;//当前分段时长
                            mpsdk.Env.onlineTimeTotal = mpsdk.Env.onlineTimeTotal + 1; //累计时长
                            mpsdk.Env.onlineTimeCount = mpsdk.Env.onlineTimeCount + 1; //当前分段时长
                            onlineTimeTotal_1 = mpsdk.Env.onlineTimeTotal;
                            onlineTimeCount_1 = mpsdk.Env.onlineTimeCount;
                        ...
                        case 1:
                            ...
                            mpsdk.Platform.instance.httpRequest(mpsdk.Env.publicServer + '/OnlineTimeReport/data/report.action', data).then(function (res) {
                                ...
                                mpsdk.Platform.instance.setStorage('online_time_count', 0); // 重置分段时长
...
(function (mpsdk) {
    ...
    var SNS = /** @class */ (function () {
         ...
         * @param type 默认为0（0：邀请新玩家进入游戏，终身只能接受一次邀请；1：每日邀请当天还没有进入过游戏的玩家进入游戏；2：每日可被邀请多次）
...
(function (mpsdk) {
     ...
     * @deprecated 请使用commonShare方法
        ...
        Share.genShareInfo = function (defaultShareInfo) {
            ...
            if (defaultShareInfo.imageId && this.shareInfoList && this.shareInfoList.length) {
                ...
                if (specialShareImage) {
                    ...
                    shareInfo_1.title = shareInfo_1.nickName ? specialShareImage.text.replace('{nickname}', shareInfo_1.nickName) : specialShareImage.text;
        ...
        //  * @returns 线上配置到分享内容
        //  */
        // public static genShareInfo(defaultShareInfo: IShareInfo): IShareInfo {
        //   // 如果指明了图片ID则精确查找
        //   if (defaultShareInfo.imageId && this.shareInfoList && this.shareInfoList.length) {
        //     // 查找图片
        //     let specialShareImage: IShareImage | undefined;
        //     for (let i = 0; i < this.shareInfoList.length; i++) {
        //       if (this.shareInfoList[i].id == defaultShareInfo.imageId) {
        //         specialShareImage = this.shareInfoList[i];
        //         break;
        //       }
        ...
        //     // 找到图片后直接返回
        //     if (specialShareImage) {
        //       const shareInfo = { ...defaultShareInfo };
        //       shareInfo.title = shareInfo.nickName ? specialShareImage.text.replace('{nickname}', shareInfo.nickName) : specialShareImage.text;
        //       shareInfo.image = specialShareImage.image;
        //       shareInfo.imageId = specialShareImage.id;
        //       delete shareInfo.nickName;
        //       delete shareInfo.scoreValue;
        //       log('生成固定分享内容成功', JSON.stringify(shareInfo));
        //       return shareInfo;
        //     }
        //   }
        //   defaultShareInfo.imageId = 0;
        //   if (!this.shareInfoList || !this.shareInfoList.length || typeof this.shareInfoList != 'object') {
        //     log('获取随机分享信息失败，使用默认分享信息');
        //     return defaultShareInfo;
        //   }
        //   // 过滤
        //   const shareImageList = this.shareInfoList.filter(item => {
        //     /////////////////////////////////////////
        //     // 如果审核状态未就绪则不做任何过滤
        //     if (!Hack.appStatus) {
        //       return true;
        ...
        //     /////////////////////////////////////////
        //     // 图片包含敏感信息 并且 当前处于审核期间
        //     if (Number(item.illegal) === 1 && Number(Hack.appStatus.level) === 0) {
        //       log('分享图', item.id, '不符合审核要求，被过滤');
        //       return false;
        //     }
        //     /////////////////////////////////////////
        //     // 地区限制
        //     if (item.city) {
        //       const currentCountry: string = utils.value(Hack.appStatus, 'ipArea.country');//用户当前国家
        //       const currentCity: string = utils.value(Hack.appStatus, 'ipArea.city');//用户当前城市
        //       // 不在中国则一定屏蔽
        //       if (currentCountry != '中国') {
        //         log('分享图', item.id, '仅限中国大陆地区使用');
        //         return false;
        //       }
        //       if (currentCity && item.city.indexOf(currentCity) != -1) {
        //         log('分享图', item.id, '未对', currentCity, '地区用户开放');
        //         return false;
        //       }
        //     }
        //     /////////////////////////////////////////
        //     // 分值限制
        //     const currentScore = defaultShareInfo.scoreValue || 0;// 用户当前分值
        //     const limitScore: number = Number(item.number) || 0;// 服务端设定的分值
        //     if (currentScore < limitScore) {
        //       log('分享图', item.id, '未对当前玩家开放');
        //       return false;
        //     }
        //     /////////////////////////////////////////
        //     // 默认不屏蔽
        ...
        //   }, this);
        //   if(!shareImageList.length&&!defaultShareInfo.version){
        //     let shareList = this.shareInfoList.filter(item => {
        //       return true;
        //     }, this);
        //   }
        //   // 容错
        //   if (!shareImageList.length) {
        //     log('随机分享被完全过滤，使用默认分享信息');
        //     return defaultShareInfo;
        //   }
        //   // 随机挑选一张图片
        //   const shareImage: IShareImage = utils.tableAlgorithm(shareImageList, 'table');
        //   if (!shareImage) {
        //     return defaultShareInfo;
        //   }
        //   // 最后整理
...
(function (mpsdk) {
    ...
    function init(gameId, gamePath, options, isNeedUnionid) {
        ...
        // 启动在线时长统计
        mpsdk.Env.onlineTimeTotal = Number(mpsdk.Platform.instance.getStorage('online_time_total') || 0);
        mpsdk.Env.onlineTimeCount = Number(mpsdk.Platform.instance.getStorage('online_time_count') || 0);
    ...
    (function (gif) {
        ...
        var LayaGifAnim = /** @class */ (function () {
            ...
            LayaGifAnim.prototype.Init = function (jsonlist, tx, titleurl, iconname, isBold) {
                ...
                if (titleurl) {
                    this._title.loadImage(titleurl);
                }
        ...
        var CCADGridUI = /** @class */ (function () {
            ...
            CCADGridUI.setItemNameNode = function (itemData, itemNode) {
                ...
                if (cc.isValid(itemNode)) {
                    ...
                    var name_1 = itemData['title'];
                    var nameBg = itemData['title_bg'];
                    var isBold = itemData['is_bold']; //1不加粗；2加粗
                    if (name_1 && name_1.length > 5) {
                        name_1 = name_1.substring(0, 4) + '...';
                    }
                    ...
                    nameLabelNode.getComponent(cc.RichText).string = name_1;
                }
            };
